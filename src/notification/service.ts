...
export async function notify(bot: Bot, chatId: string, notification: typeof NotificationSchema.static) {
  ...
  const files = attachmentBytes.map((file, i) => InputMediaBuilder.document(new InputFile(file, attachments[i]?.name)));
